const { roles, colaboradores } = require('../../database/models')
...
const postCollaborators = async (req, res) => {
    try {
        ...
        const findCollaborators = await colaboradores.findOne(
            {
                where: {
                    numeroDocumento,
                    activo: 1
        ...
        if (findCollaborators) {
            ...
            const data = await colaboradores.create({
                ...req.body,
...
const getCollaborators = async (req, res) => {
    try {
        const data = await colaboradores.findAll(
            {
                where: {
                    activo: 1
                }
...
const putCollaborators = async (req, res) => {
    try {
        console.log("req.body", req.body)
        const data = await colaboradores.update(
            { ...req.body }, {
            where: {
                id: req.body.id
            }
...
const loginCollaborators = async (req, res) => {
    try {
        ...
        const data = await colaboradores.findOne(
            {
                where: {
