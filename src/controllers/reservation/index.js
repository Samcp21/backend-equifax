...
const getPackages = async (req, res) => {
    try {
        ...
        console.log('idReserva_getPackages', idReserva)
...
const filterForPackages = async (req, res) => {
    try {
        ...
        const { idColaborador, date, idCliente } = req.body
        let filterDate = ''
        if (date) {
            filterDate = `and r.fechaCreacion like '${date}%'`
        }
        console.log('filterDate', filterDate)
        const data = await Sequelize.query(obtenerPackage.concat('', filterDate), {
            replacements: {
                idCollaborator: idColaborador || null,
                idPerson: idCliente || null,
            },
...
const findPackagesReservation = async (req, res) => {
    try {
        ...
        let filterDate = ''
        if (date) {
            filterDate = `and r.fechaCreacion  = IFNULL("${date}%",r.fechaCreacion) `
        }
        console.log('filterDate', filterDate)
        const data = await Sequelize.query(obtenerPackage.concat(' ', filterDate), {
            replacements: {
